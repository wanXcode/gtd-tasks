...
from sync_agent_mac import CATEGORY_TO_LIST, BUCKET_TO_LIST, render_reminder_note, run_reminders_backend  # noqa: E402
...
def main() -> int:
    ...
    for task_id, reminder_id in mappings.items():
        ...
        note = render_reminder_note(task.get('note', '') or '', list(task.get('tags') or []))
        ...
        try:
            run_reminders_backend('update', reminder_id=reminder_id, title=title, note=note)
